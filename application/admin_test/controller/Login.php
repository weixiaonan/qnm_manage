...
class Login extends Controller
{
    ...
    public function index()
    {
        ...
        strlen($username) < 2 && $this->error('登录账号长度不能少于2位有效字符!');
        strlen($password) < 2 && $this->error('登录密码长度不能少于2位有效字符!');
        // 用户信息验证
        $user = Db::name('SystemUser')->where('username', $username)->whereOr('phone', $username)->find();
        ...
        $q = Db::name('SystemUser')->where(['id' => $user['id']])->update($data);
        $q = Db::name('SystemUser')->where(['id' => $user['id']])->setInc('login_num');
        session('user', $user);
